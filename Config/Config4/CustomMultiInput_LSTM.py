...
class CustomMultiInputLSTM(nn.Module):
    ...
    def forward(self, Y, x4, x5, x6, x7):
        ...
        for t in range(seq_sz):
            ...
            alpha_t = torch.softmax(torch.stack([u_t, u_x4_t, u_x5_t, u_x6_t, u_x7_t]),dim = 0)
            # After Attention Layer
            L_t = alpha_t[0, :, :] * l_t + alpha_t[1, :, :] * l_x4_t + alpha_t[2, :, :] * l_x5_t + alpha_t[3, :,:] * l_x6_t + alpha_t[4, :,:] * l_x7_t
            """+ alpha_t[
                                                                                                                 4, :,
                                                                                                                 :] * l_x4_t + alpha_t[
                                                                                                                               5,
