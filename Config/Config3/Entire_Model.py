...
class Net(nn.Module):
    ...
    def forward(self, Y, x1, x2, x3):
        # Executed when input is passed into the neural network
        output, hidden = self.MI_LSTM_layer(Y, x1, x2, x3)
...
for epoch in range(10):
    ...
    # T = 22
    for Y, y, x1, x2, x3 in loader:
        optimizer.zero_grad()
        # Shapes:  Y = (1, 5, 22); x1 = (1, 3 ,22) (bei 3 Kennzahlen pro Klasse); x2 = ...

        outputs = net(Y.float(), x1.float(), x2.float(), x3.float())
