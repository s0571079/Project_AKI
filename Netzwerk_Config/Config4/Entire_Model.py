...
class Net(nn.Module):
    ...
    def forward(self, Y, x4, x5, x6, x7):
        # Executed when input is passed into the neural network
        output, hidden = self.MI_LSTM_layer(Y, x4, x5, x6, x7)
...
for epoch in range(10):
    ...
    # T = 22
    for Y, y, x4, x5, x6, x7 in loader:
        optimizer.zero_grad()
        # Shapes:  Y = (1, 5, 22); x1 = (1, 3 ,22) (bei 3 Kennzahlen pro Klasse); x2 = ...

        outputs = net(Y.float(), x4.float(), x5.float(), x6.float(), x7.float())
...
#Plot results
plt.plot(range(10), loss_plot_values)
plt.ylabel('Fehler L1')
plt.xlabel('Anzahl Durchläufe')
plt.show()